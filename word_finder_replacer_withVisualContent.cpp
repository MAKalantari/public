...
#include <iostream>
#include <windows.h> //"windows.h" is only included for visual purposes.
...
int main()
{
    ...
    {
        if (i == highlight.mainArr[pos])
        {
            for (int n = i; n <= highlight.mainArr[pos+1]; n++)
            {
                SetConsoleTextAttribute(hConsole, 240);
                cout << str[n];
            }
            i += highlight.mainArr[pos+1] - highlight.mainArr[pos];
            pos += 2;
        }
        else {
